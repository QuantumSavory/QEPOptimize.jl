...
function unsafe_cleanup_nonmeasurement_last_steps!(ops,num_pairs,num_purified)
    ...
    while notDone
        ...
        qubits_who_need_measurements = reduce((arr, (qubit, op_index)) ->
                ...
                || typeof(ops[op_index]) <: AbstractMeasurement) ? arr : # and filter out measurements
...
"""
    ...
    safe=true # true to only improve circuits, false to include canonicalization steps that may make some circuits worse/make experimental changes to try to improve them
    )

Apply all of the canonicalization cleanup methods to a population with tmap. Defaults to safe canonicalization steps.
"""
function canonicalize_cleanup!(pop::Population,num_pairs,num_purified;
    safe=true # :all (include canonicalization steps that may make some circuits worse/make experimental changes to try to improve) or :safe (only improve circuits)
    )
    # Function to change the ops based on the canonicalization steps requested
    cleanup! = safe ?
